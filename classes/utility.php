...
class SLFW_Util {
	...
	*/
	static public  function SSDirURL() {
		 return get_site_url(null, '', is_ssl() ? 'https' : 'http') . '/' . FLIPWALL_SSDIR_NAME . '/';
	}
	...
	*/
	static public function RunAPC() {
	   if(function_exists('apc_compile_file')){
		   $file01 = @apc_compile_file(FLIPWALL_PLUGIN_PATH . "flipwall.php");
		   return ($file01);
	   } else {
		   return false;
	   }
	...
	static public function ListDirs($path = '.') {
		...
		foreach (new DirectoryIterator($path) as $file) {
			if ($file->isDir() && !$file->isDot()) {
				$dirs[] = SLFW_Util::SafePath($file->getPathname());
			}
		}
		return $dirs;
	}
	...
	*/
	static public function InitSnapshotDirectory() {
		$path_wproot	= SLFW_Util::SafePath(FLIPWALL_WPROOTPATH);
		$path_ssdir		= SLFW_Util::SafePath(FLIPWALL_SSDIR_PATH);
		$path_plugin	= SLFW_Util::SafePath(FLIPWALL_PLUGIN_PATH);
