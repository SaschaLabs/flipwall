...
class SLFW_Database {
	...
	public function Build($package) {
		try {
			...
			$time_start = SLFW_Util::GetMicrotime();
			$this->Package->SetStatus(SLFW_PackageStatus::DBSTART);
			...
			$log .= "MYSQLTIMEOUT: " . FLIPWALL_DB_MAX_TIME;
			SLFW_Log::Info($log);
			...
			SLFW_Log::Info("SQL CREATED: {$this->File}");
			$time_end = SLFW_Util::GetMicrotime();
			$time_sum = SLFW_Util::ElapsedTime($time_end, $time_start);

			$sql_file_size = filesize($this->dbStorePath);
			if ($sql_file_size <= 0) {
				SLFW_Log::Error("SQL file generated zero bytes.", "No data was written to the sql file.  Check permission on file and parent directory at [{$this->dbStorePath}]");
			}
			SLFW_Log::Info("SQL FILE SIZE: " . SLFW_Util::ByteSize($sql_file_size));
			SLFW_Log::Info("SQL RUNTIME: {$time_sum}");

			$this->Size = @filesize($this->dbStorePath);
			$this->Package->SetStatus(SLFW_PackageStatus::DBDONE);
			
		} catch (Exception $e) {
			SLFW_Log::Error("Runtime error in SLFW_Database::Build","Exception: {$e}");
		}
	}
	...
	public function Stats() {
		...
		foreach ($tables as $table) {
			...
			$info['TableList'][$name]['Size']	= SLFW_Util::ByteSize($size);
			$tblCount++;
		}
	...
	public static function GetMySqlDumpPath() {
		...
		if (! SLFW_Util::IsShellExecAvailable()) {
			return false;
		}

		$custom_mysqldump_path	= SLFW_Settings::Get('package_mysqldump_path');
		$custom_mysqldump_path = (strlen($custom_mysqldump_path)) ? $custom_mysqldump_path : '';
		
		//Common Windows Paths
		if (SLFW_Util::IsOSWindows()) {
			$paths = array(
	...
	private function mysqlDump($exePath) {
		...
		SLFW_Log::Info("TABLES: total:{$tblAllCount} | filtered:{$tblFilterCount} | create:{$tblCreateCount}");
		SLFW_Log::Info("FILTERED: [{$this->FilterTables}]");		
		SLFW_Log::Info("RESPONSE: {$output}");
	...
	private function phpDump() {
		...
		$wpdb->query("SET session wait_timeout = " . FLIPWALL_DB_MAX_TIME);
		...
		$sql_header = "/* FLIPWALL MYSQL SCRIPT CREATED ON : " . @date("F j, Y, g:i a") . " */\n\n";
		...
		foreach ($tables as $table) {
			//$sql_del = ($GLOBALS['FLIPWALL_opts']['dbadd_drop']) ? "DROP TABLE IF EXISTS {$table};\n\n" : "";
