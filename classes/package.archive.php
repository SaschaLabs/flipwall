...
require_once (FLIPWALL_PLUGIN_PATH . 'classes/package.archive.zip.php');
...
class SLFW_Archive {
	...
	public function Build($package) {
		try {
			...
			$this->Package->SetStatus(SLFW_PackageStatus::ARCSTART);
			switch ($this->Format) {
				case 'TAR':			break;
				case 'TAR-GZIP': 	break;
				default:
			...
			$this->Package->SetStatus(SLFW_PackageStatus::ARCDONE);
	...
	public function GetFilterDirAsArray() {
		return array_map('SLFW_Util::SafePath', explode(";", $this->FilterDirs, -1));
	}
	...
	private function getDirs() {
		...
		$rootPath = SLFW_Util::SafePath(rtrim(FLIPWALL_WPROOTPATH, '//' ));
		array_push($this->filterDirsArray, FLIPWALL_SSDIR_PATH);
		...
		if (in_array($this->PackDir, $this->filterDirsArray)) {
			...
			$this->Dirs = (FLIPWALL_SCAN_USELEGACY)
			...
			foreach (glob("{$val}/{,.}*", GLOB_NOSORT | GLOB_BRACE) as $filePath) {
				...
				if (!is_dir($filePath)){
					...
					if (!in_array(@pathinfo($filePath, PATHINFO_EXTENSION), $this->filterExtsArray)  && is_readable($filePath)) {
						...
						if ($fileSize > FLIPWALL_SCAN_WARNFILESIZE) {
							array_push($this->WarnFileSize, $filePath . ' [' . SLFW_Util::ByteSize($fileSize) . ']');
						}
						if ($valid) {
							$this->Size += $fileSize;
							$this->Files[] = $filePath;
	...
	private function dirsToArray_Legacy($path) {
		...
		if ($handle) {
			while (($file = readdir($handle)) !== false ) {
				if ($file != "." && $file != "..") {
					$fullPath = SLFW_Util::SafePath($path. "/" . $file);
					if (is_dir($fullPath)) {
	...
	private function dirsToArray_New($path) {
		...
		foreach ($files as $file) {
			if ($file->isDir()) {
				$items[] = SLFW_Util::SafePath($file->getRealPath());
			}
		}
		return $items;
	}
