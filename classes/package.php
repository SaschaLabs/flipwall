...
require_once (FLIPWALL_PLUGIN_PATH . 'classes/package.installer.php');
require_once (FLIPWALL_PLUGIN_PATH . 'classes/package.database.php');
require_once (FLIPWALL_PLUGIN_PATH . 'classes/utility.php');

final class SLFW_PackageStatus {
...
class SLFW_Package {
	...
	const OPT_ACTIVE   = 'FLIPWALL_package_active';
    ...
    function __construct() {
		...
		$this->ID			= null;
		$this->Version		= FLIPWALL_VERSION;
		$this->Type			= SLFW_PackageType::MANUAL;
		$this->Name			= self::GetDefaultName();
		$this->Notes		= null;
		$this->StoreURL     = SLFW_Util::SSDirURL();
		$this->StorePath    = FLIPWALL_SSDIR_PATH_TMP;
		$this->Database		= new SLFW_Database($this);
		$this->Archive		= new SLFW_Archive($this);
		$this->Installer	= new SLFW_Installer($this);
	...
	public function Scan() {
		...
		$timerStart = SLFW_Util::GetMicrotime();
		$report = array();
		...
		//FILES
		$this->Archive->Stats();
		$report['ARC']['Size']				= SLFW_Util::ByteSize($this->Archive->Size)  or "unknown";
		...
		$report['ARC']['Status']['Size']	= ($this->Archive->Size > FLIPWALL_SCAN_SITE) ? 'Warn' : 'Good';
		...
		$report['DB']['Size']		= SLFW_Util::ByteSize($db['Size'])	or "unknown";
		...
		$report['RPT']['ScanTime'] = SLFW_Util::ElapsedTime(SLFW_Util::GetMicrotime(), $timerStart);
		$fp = fopen(FLIPWALL_SSDIR_PATH_TMP . "/{$this->ScanFile}", 'w');
	...
	public function Build() {
		...
		//START LOGGING
		SLFW_Log::Open($this->NameHash);
		$php_max_time	= @ini_get("max_execution_time");
		$php_max_memory = @ini_set('memory_limit', FLIPWALL_PHP_MAX_MEMORY);
		$php_max_time	= ($php_max_time == 0)        ? "(0) no time limit imposed" : "[{$php_max_time}] not allowed";
		$php_max_memory = ($php_max_memory === false) ? "Unabled to set php memory_limit" :  FLIPWALL_PHP_MAX_MEMORY . " ({$php_max_memory} default)";
		
		$info  = "********************************************************************************\n";
		$info .= "PACKAGE-LOG: " . @date("Y-m-d H:i:s") . "\n";
		$info .= "NOTICE: Do NOT post to public sites or forums \n";
		$info .= "********************************************************************************\n";
		$info .= "VERSION:\t" . FLIPWALL_VERSION . "\n";
		...
		if (! $packageObj) {
			SLFW_Log::Error("Unable to serialize pacakge object while building record.");
		}
		
		$this->ID = $this->FindHashKey($this->Hash);
		
		if ($this->ID != 0){
			 $this->SetStatus(SLFW_PackageStatus::START);
		} else {
			$results = $wpdb->insert($wpdb->prefix . "FLIPWALL_packages", array(
				'name'    => $this->Name,
				'hash'	  => $this->Hash,
				'status'  => SLFW_PackageStatus::START,
			...
			if ($results == false) {
				...
				SLFW_Log::Error("Unable to insert record into database table.", "'{$error_result}'");
			}
		...
		//VALIDATE FILE SIZE
		$dbSizeRead	 = SLFW_Util::ByteSize($this->Database->Size);
		$zipSizeRead = SLFW_Util::ByteSize($this->Archive->Size);
		$exeSizeRead = SLFW_Util::ByteSize($this->Installer->Size);
		if ( !($this->Archive->Size && $this->Database->Size && $this->Installer->Size)) {
			SLFW_Log::Error("A required file contains zero bytes.", "Archive Size: {$zipSizeRead} | SQL Size: {$dbSizeRead} | Installer Size: {$exeSizeRead}");
		}

		$this->SetStatus(SLFW_PackageStatus::COMPLETE);
		$timerEnd = SLFW_Util::GetMicrotime();
		$timerSum = SLFW_Util::ElapsedTime($timerEnd, $timerStart);
	...
	public function SaveActive($post = null) {
		...
		if (isset($post)) {
			...
			//ARCHIVE
			$this->Archive->PackDir			= rtrim(FLIPWALL_WPROOTPATH, '/');
	...
	public function SetStatus($status) {
		...
		if (! isset($status)) {
			SLFW_Log::Error("Package SetStatus did not receive a proper code.");
		}

		if (! $packageObj) {
			SLFW_Log::Error("Package SetStatus was unable to serialize package object while updating record.");
		}
		
		$wpdb->flush();
		$table = $wpdb->prefix . "FLIPWALL_packages";
	 ...
	 * @see SLFW_Package::SaveActive
	 * @return SLFW_Package
	 */
	public static function GetActive() {
		
		global $wpdb;
		$obj = new SLFW_Package();
		...
		$obj = (is_object($obj)) ? $obj : new SLFW_Package();
		return $obj;
	}
	...
	*/
	public static function TmpCleanup($all = false) {
		...
		//Remove scan files that are 24 hours old
			...
			$dir = FLIPWALL_SSDIR_PATH_TMP . "/*_scan.json";
			foreach (glob($dir) as $file) {
	...
	private function buildCleanup() {
		...
		$files   = glob(FLIPWALL_SSDIR_PATH_TMP . "/{{$this->NameHash}*}", GLOB_BRACE | GLOB_NOSORT);
	...
	private function parseDirectoryFilter($dirs = "") {
		...
		foreach (explode(";", $dirs) as $val) {
			if (strlen($val) >= 2) {
				$filter_dirs .= SLFW_Util::SafePath(trim(rtrim($val, "/\\"))) . ";";
			}
		}
		return $filter_dirs;
	}
	...
	private function parseExtensionFilter($extensions = "") {
		...
		if (strlen($extensions) >= 1 && $extensions != ";") {
			...
			$filter_exts   = SLFW_Util::StringAppend($extensions, ";");
		}
		return $filter_exts;
	}
