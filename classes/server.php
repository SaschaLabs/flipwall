...
class SLFW_Server {
	...
	*/
	public static function GetRequirments() {
		...
		$req = array();
		//SYS-100: FILE PERMS
		$test = is_writeable(FLIPWALL_WPROOTPATH)
				&& is_writeable(FLIPWALL_SSDIR_PATH)
				&& is_writeable(FLIPWALL_SSDIR_PATH_TMP);
	...
	*/
	public static function GetChecks() {
		...
		$php_test2  = ($php_test2  > FLIPWALL_SCAN_TIMEOUT || strcmp($php_test2 , 'Off') == 0 || $php_test2  == 0) ? 'Good' : 'Warn';
		$checks['CHK-SRV-100'] = ($php_test1 && $php_test2) ? 'Good' : 'Warn';

		//CHK-SRV-101: WORDPRESS SETTINGS
		//Version
		global $wp_version;
		$version_test = version_compare($wp_version,  FLIPWALL_SCAN_MIN_WP) >= 0 ? true : false;
		
		//Cache
		$cache_path = SLFW_Util::SafePath(WP_CONTENT_DIR) .  '/cache';
		$dirEmpty	= SLFW_Util::IsDirectoryEmpty($cache_path);
		$dirSize	= SLFW_Util::GetDirectorySize($cache_path); 
		$cache_test = ($dirEmpty  || $dirSize < FLIPWALL_SCAN_CACHESIZE ) ? true : false;
		
		//Core Files
		$files = array();
		$files['wp-config.php'] = file_exists(SLFW_Util::SafePath(FLIPWALL_WPROOTPATH .  '/wp-config.php'));
		$files_test = $files['wp-config.php'];
		
		$checks['CHK-SRV-101'] = $files_test && $cache_test && $version_test ? 'Good' : 'Warn';

		//CHK-SRV-102: WEB SERVER 
		$servers = $GLOBALS['FLIPWALL_SERVER_LIST'];
		$test = false;
	...
	/** 
	* Check to see if flipwall installer files are present
	* @return bool   True if any reserved files are found
	*/
	public static function InstallerFilesFound() {
		
		$phpFile = file_exists(FLIPWALL_WPROOTPATH . FLIPWALL_INSTALL_PHP);
		$sqlFile = file_exists(FLIPWALL_WPROOTPATH . FLIPWALL_INSTALL_SQL);
		$logFile = file_exists(FLIPWALL_WPROOTPATH . FLIPWALL_INSTALL_LOG);
