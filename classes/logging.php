...
class SLFW_Log {
	...
	static public function Open($name) {
		...
		self::$logFileHandle = @fopen(FLIPWALL_SSDIR_PATH . "/{$name}.log", "c+");	
	}
	...
	*/
	static public function Error($msg, $detail) {
		...
		$err_msg  = "\n==================================================================================\n";
		$err_msg .= "!!!FLIPWALL ERROR!!!\n";
		$err_msg .= "Please Try Again! If the error persists please see the Flipwall 'Support' link.\n";
		$err_msg .= "---------------------------------------------------------------------------------\n";
		$err_msg .= "MESSAGE:\n{$msg}\n";
		if (strlen($detail)) {
			$err_msg .= "DETAILS:\n{$detail}\n";
		...
		die("FLIPWALL ERROR: Please see the flipwall log file.");
	}
