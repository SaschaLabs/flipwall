...
class SLFW_Installer {
	...
	public function Build($package) {
		...
		SLFW_Log::Info("\n********************************************************************************");
		SLFW_Log::Info("INSTALLER:");
		SLFW_Log::Info("********************************************************************************");
		SLFW_Log::Info("INSTALLER FILE: Build Start");

		$template_uniqid = uniqid('') . '_' . time();
		$template_path	= SLFW_Util::SafePath(FLIPWALL_SSDIR_PATH_TMP  . "/installer.template_{$template_uniqid}.php");
		$main_path		= SLFW_Util::SafePath(FLIPWALL_PLUGIN_PATH . 'installer/build/main.installer.php');
		...
		if ($main_data === false || $template_result == false) {
			...
			SLFW_Log::Error("Install builder failed to generate files.", "{$err_info}");
		}
		...
		foreach ($embeded_files as $name => $token) {
			$file_path = FLIPWALL_PLUGIN_PATH . "installer/build/{$name}";
			...
			if ($search_data === false || $insert_data == false) {
				SLFW_Log::Error("Installer generation failed at {$token}.");
			}
		...
		SLFW_Log::Info("INSTALLER FILE: Build Finished");
	...
	*/
	private function addBackup() {
		...
		$zipPath   = SLFW_Util::SafePath("{$this->Package->StorePath}/{$this->Package->Archive->File}");
		$installer = SLFW_Util::SafePath(FLIPWALL_SSDIR_PATH_TMP) . "/{$this->Package->NameHash}_installer.php";
		
		$zipArchive	= new ZipArchive();
		if ($zipArchive->open($zipPath, ZIPARCHIVE::CREATE) === TRUE) {
			if ($zipArchive->addFile($installer, "installer-backup.php"))  {
				SLFW_Log::Info("INSTALLER FILE: Added to archive");
			} else {
				SLFW_Log::Info("Unable to add installer-backup.php to archive.", "Installer File Path [{$installer}]");
			}
			$zipArchive->close();
		}
	}
	...
	*/
	private function createFromTemplate($template) {
		...
		$replace_items = Array(
			...
			"fwrite_wproot"				=> FLIPWALL_WPROOTPATH,
			"fwrite_FLIPWALL_version" => FLIPWALL_VERSION);
		...
		if (file_exists($template) && is_readable($template)) {
			...
			(empty($install_str))
					? SLFW_Log::Error("{$err_msg}" , "SLFW_Installer::createFromTemplate => file-empty-read")
					: SLFW_Log::Info("INSTALLER FILE: Template parsed with new data");
			
			//INSTALLER FILE
			$fp = (!file_exists($installer)) ? fopen($installer, 'x+') : fopen($installer, 'w');
			if (! $fp || ! fwrite($fp, $install_str, strlen($install_str))) {
				SLFW_Log::Error("{$err_msg}", "SLFW_Installer::createFromTemplate => file-write-error");
			}

			@fclose($fp);
		} else {
			SLFW_Log::Error("Installer Template missing or unreadable.", "Template [{$template}]");
		}
		@unlink($template);
		SLFW_Log::Info("INSTALLER FILE: Complete [{$installer}]");
	}
