<?php
	$qryResult = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}FLIPWALL_packages` ORDER BY id DESC", ARRAY_A);
	$qryStatus = $wpdb->get_results("SELECT status FROM `{$wpdb->prefix}FLIPWALL_packages` WHERE status >= 100", ARRAY_A);
...
<form id="form-flipwall" method="post">
...
<?php if($totalElements == 0)  :	?>
	...
	<table class="widefat dup-pack-table" style="margin-top:20px">
		...
		<tbody><tr><td><?php include (FLIPWALL_PLUGIN_PATH .  "views/packages/list-nodata.php") ?> </td></tr></tbody>
...
<?php else : ?>	
			...
			<td style="white-space: nowrap">
				<div class="alignleft actions">
					<select id="dup-pack-bulk-actions">
						<option value="-1" selected="selected"><?php _e("Bulk Actions", 'wpflipwall') ?></option>
						<option value="delete" title="<?php _e("Delete selected package(s)", 'wpflipwall') ?>"><?php _e("Delete", 'wpflipwall') ?></option>
					</select>
					<input type="button" id="dup-pack-bulk-apply" class="button action" value="<?php _e("Apply", 'wpflipwall') ?>" onclick="Flipwall.Pack.Delete()">
				</div>
				<br class="clear">
			</td>
			<!-- Logs -->
			<td><img src="<?php echo FLIPWALL_PLUGIN_URL  ?>assets/img/hdivider.png" class="toolbar-divider" /></td>
			<td align="center">
				<a href="?page=flipwall-tools" id="btn-logs-dialog" class="button"  title="<?php _e("Package Logs", 'wpflipwall') ?>..."><i class="fa fa-pencil-square-o"></i> </button>
			</td>
			<td align="right" style="width:100%;">
				<?php if($statusCount >= 3)  :	?>
				<div style="margin:8px 5px 0px 0px; font-size:12px">
					<a href="admin.php?page=flipwall-support"><?php _e("Help Support Flipwall", 'wpflipwall') ?> <i class="fa fa-check-circle"></i> </a>
				</div>
				<?php endif; ?>	
			</td>
		</tr>
	...
	<table class="widefat dup-pack-table">
		<thead>
			<tr>
				<th><input type="checkbox" id="dup-bulk-action-all"  title="<?php _e("Select all packages", 'wpflipwall') ?>" style="margin-left:15px" onclick="Flipwall.Pack.SetDeleteAll()" /></th>
				<th><?php _e("Details", 'wpflipwall') ?></th>
				<th><?php _e("Created", 'wpflipwall') ?></th>
				<th><?php _e("Size", 'wpflipwall') ?></th>
				<th style="width:90%;"><?php _e("Name", 'wpflipwall') ?></th>
				<th style="text-align:center;" colspan="2">
					<?php _e("Package",  'wpflipwall')?>
				</th>
			</tr>
		</thead>
		<?php
		...
		foreach ($rows as $row) {
			...
			<?php if ($row['status'] >= 100) : ?>
				<tr class="dup-pack-info <?php echo $css_alt ?>">
					...
					<td><a href="javascript:void(0);" onclick="return Flipwall.Pack.ToggleDetail('<?php echo $detail_id ;?>');">[<?php echo __("View", 'wpflipwall') . ' ' . $row['id'];?>]</a></td>
					<td><?php echo date( "m-d-y G:i", strtotime($row['created']));?></td>
					<td><?php echo SLFW_Util::ByteSize($pack_archive_size); ?></td>
					<td class='pack-name'><?php	echo  $pack_name ;?></td>
					<td class="get-btns">
						<button id="<?php echo "{$uniqueid}_installer.php" ?>" class="button no-select" onclick="Flipwall.Pack.DownloadFile('<?php echo $installfilelink; ?>', this); return false;"><i class="fa fa-bolt"></i> <?php _e("Installer", 'wpflipwall') ?></button> &nbsp;
					</td>
					<td class="get-btns">	
						<button id="<?php echo "{$uniqueid}_archive.zip" ?>" class="button no-select" onclick="Flipwall.Pack.DownloadFile('<?php echo $packagepath; ?>', this); return false;"><i class="fa fa-file-archive-o"></i> <?php _e("Archive", 'wpflipwall') ?></button>
					</td>
				</tr>
				<tr>
					<td colspan="8" id="<?php echo $detail_id; ?>" class="dup-pack-details  <?php echo $css_alt ?>">
						<b><?php _e("Version", 'wpflipwall') ?>:</b> <?php echo $plugin_version ?> &nbsp; | &nbsp; 
						<b><?php _e("User", 'wpflipwall') ?>:</b> <?php echo $row['owner']; ?> &nbsp; | &nbsp;  
						<b><?php _e("Hash", 'wpflipwall')?>:</b> <?php echo $pack_namehash ;?> <br/>
						<b><?php _e("Notes", 'wpflipwall')?>:</b> <?php echo $notes ?> 
						<div style="height:7px">&nbsp;</div>
						<button class="button" onclick="Flipwall.Pack.ShowLinksDialog(<?php echo "'{$sqlfilelink}', '{$packagepath}', '{$installfilelink}', '{$logfilelink}', '{$reportfilelink}' " ;?>); return false;" class="thickbox"><i class="fa fa-lock"></i> &nbsp; <?php _e("Links", 'wpflipwall')?></button> &nbsp; 
						<button class="button" onclick="window.open(<?php echo "'{$sqlfilelink}', '_blank'" ;?>); return false;"><i class="fa fa-table"></i> &nbsp; <?php _e("SQL File", 'wpflipwall')?></button> &nbsp; 
						<button class="button" onclick="Flipwall.OpenLogWindow(<?php echo "'{$logfilename}'" ;?>); return false;"><i class="fa fa-pencil-square-o"></i> &nbsp; <?php _e("View Log", 'wpflipwall')?></button>
						<?php if ($package_debug) : ?>
							<div style="margin-top:7px">
								<a href="javascript:void(0)" onclick="window.open(<?php echo "'{$reportfilelink}', '_blank'" ;?>); return false;">[<?php _e("Open Scan Report", 'wpflipwall')?>]</a> &nbsp;
								<a href="javascript:void(0)" onclick="jQuery(this).parent().find('.dup-pack-debug').toggle()">[<?php _e("View Package Object", 'wpflipwall')?>]</a><br/>
			...
			<?php else : ?>	
				...
				<?php
					$size = 0;
					$tmpSearch = glob(FLIPWALL_SSDIR_PATH_TMP . "/{$pack_namehash}_*");
				...
				<tr class="dup-pack-info  <?php echo $css_alt ?>">
					...
					<td><a href="javascript:void(0);" onclick="return Flipwall.Pack.ToggleDetail('<?php echo $detail_id ;?>');">[<?php echo __("View", 'wpflipwall') . ' ' . $row['id'];?>]</a></td>
					<td><?php echo date( "m-d-y G:i", strtotime($row['created']));?></td>
					<td><?php echo SLFW_Util::ByteSize($size); ?></td>
					<td class='pack-name'><?php echo $pack_name ;?></td>
					<td class="get-btns" colspan="2">		
						<span style='display:inline-block; padding:7px 40px 0px 0px'>
							<a href="javascript:void(0);" onclick="return Flipwall.Pack.ToggleDetail('<?php echo $detail_id ;?>');"><?php _e("View Error Details", 'wpflipwall') ?>...</a>
						</span>									
					</td>
				</tr>
				<tr>
					<td colspan="8" id="<?php echo $detail_id; ?>" class="dup-pack-details  <?php echo $css_alt ?>">
						<div class="dup-details-area-error">
							<b><?php _e("Version", 'wpflipwall') ?>:</b> <?php echo $plugin_version ?> &nbsp; | &nbsp; 
							<b><?php _e("User", 'wpflipwall') ?>:</b> <?php echo $row['owner']; ?> &nbsp; | &nbsp;  
							<b><?php _e("Hash", 'wpflipwall')?>:</b> <?php echo $pack_name ;?> <br/>
							
							<?php if ($pack_name == 'unknown') : ?>
								<b><?php _e("Unrecoverable Error! Please remove this package.", 'wpflipwall')?></b><br/>
							<?php else : ?>	
								<b><?php _e("Notes", 'wpflipwall')?>:</b> <?php echo $notes ?> <br/>
								<?php
									printf("%s <u><a href='http://www.SaschaLabs.net/flipwall-docs' target='_blank'>%s</a></u>",
									__("This package has encountered errors.  Click 'View Log' for more details.  For additional support see the ", 'wpflipwall'),
									__("online knowledgebase", 'wpflipwall')); 
								?><div style="height:7px">&nbsp;</div>
								<button class='button' onclick="Flipwall.OpenLogWindow(<?php echo "'{$logfilename}'" ;?>); return false;"><?php _e("View Log", 'wpflipwall')?></button>
								<?php if ($package_debug) : ?>
									<div style="margin-top:7px">
	...
	<tfoot>
		<tr>
			<th colspan="8" style='text-align:right; font-size:12px'>						
				<?php echo _e("Packages", 'wpflipwall') . ': ' . $totalElements; ?> |
				<?php echo _e("Total Size", 'wpflipwall') . ': ' . SLFW_Util::ByteSize($totalSize); ?> 
			</th>
		</tr>
	</tfoot>
	</table>
...
DIALOG: QUICK PATH -->
<?php add_thickbox(); ?>
<div id="dup-dlg-quick-path" title="<?php _e('Download Links', 'wpflipwall'); ?>" style="display:none">
	<p>
		<i class="fa fa-lock"></i>
		<?php _e("The following links contain sensitive data.  Please share with caution!", 'wpflipwall');	?>
	</p>
	
	<div style="padding: 0px 15px 15px 15px;">
		<a href="javascript:void(0)" style="display:inline-block; text-align:right" onclick="Flipwall.Pack.GetLinksText()">[Select All]</a> <br/>
		...
		<i style='font-size:11px'><?php _e("The database SQL script is a quick link to your database backup script.  An exact copy is also stored in the package.", 'wpflipwall'); ?></i>
	</div>
</div>
...
jQuery(document).ready(function($) {
	...
	Flipwall.Pack.ToggleDetail = function(id) {
		$('#' + id).toggle();
		return false;
	}
	...
	Flipwall.Pack.Delete = function (event) {
		var arr = new Array;
		var count = 0;
		
		if ($("#dup-pack-bulk-actions").val() != "delete") {
			alert("<?php _e('Please select an action from the bulk action drop down menu to perform a specific action.', 'wpflipwall') ?>");
			return;
		}
		...
		var list = arr.join(',');
		if (list.length == 0) {
			alert("<?php _e('Please select at least one package to delete.', 'wpflipwall') ?>");
			return;
		}
		
		if (confirm("<?php _e('Are you sure, you want to delete the selected package(s)?', 'wpflipwall') ?>")){
			
			$.ajax({
				type: "POST",
				url: ajaxurl,
				dataType: "json",
				data: {action : 'FLIPWALL_package_delete', FLIPWALL_delid : list },
				success: function(data) { 
					//console.log(data); //Debug return
					Flipwall.ReloadWindow(data); 
				}
			});
		} 
		if (event)
	...
	Flipwall.Pack.SetDeleteAll = function() {
	...
	Flipwall.Pack.ShowLinksDialog = function(db, install, pack, log, report) {
		
		var url = '#TB_inline?width=650&height=350&inlineId=dup-dlg-quick-path';
		tb_show("<?php _e('Package File Links', 'wpflipwall') ?>", url);
		
		var msg = <?php printf('"%s:\n" + db + "\n\n%s:\n" + install + "\n\n%s:\n" + pack + "\n\n%s:\n" + log + "\n\n%s:\n" + report;', 
			__("DATABASE",  'wpflipwall'), 
			__("PACKAGE", 'wpflipwall'), 
			__("INSTALLER",   'wpflipwall'),
			__("LOG", 'wpflipwall'),
			__("REPORT", 'wpflipwall')); 
		?>
	...
	Flipwall.Pack.GetLinksText = function() {$('#dup-dlg-quick-path-data').select();};	
