<?php
	...
	$logurl	 = get_site_url(null, '', is_ssl() ? 'https' : 'http') . '/' . FLIPWALL_SSDIR_NAME . '/' . $logname;
...
jQuery(document).ready(function($) {
	...
	Flipwall.Tools.FullLog = function() {
	...
	Flipwall.Tools.Refresh = function() {
		$('#refresh').val(1);
		$('#dup-form-logs').submit();
	}
	
	Flipwall.Tools.RefreshAuto = function() {
	...
	Flipwall.Tools.GetLog = function(log) {
		window.location =  log;
	}
	
	Flipwall.Tools.WinResize = function() {
	...
	//INIT Events
	$(window).resize(Flipwall.Tools.WinResize);
	$('#dup-options').click(Flipwall.Tools.FullLog);
	$("#dup-refresh").click(Flipwall.Tools.Refresh);
	$("#dup-auto-refresh").click(Flipwall.Tools.RefreshAuto);
	$("#dup-refresh-count").html(duration.toString());
	
	//INIT
	Flipwall.Tools.WinResize();
	<?php if ($refresh)  :	?>
		//Scroll to Bottom
...
<?php if (! $logfound)  :	?>
	<div style="padding:20px">
		<h2><?php _e("Log file not found or unreadable", 'wpflipwall') ?>.</h2>

		<?php _e("Try to create a package, since no log files were found in the snapshots directory with the extension *.log", 'wpflipwall') ?>.<br/><br/>

		<?php _e("Reasons for log file not showing", 'wpflipwall') ?>: <br/>
		- <?php _e("The web server does not support returning .log file extentions", 'wpflipwall') ?>. <br/>
		- <?php _e("The snapshots directory does not have the correct permissions to write files.  Try setting the permissions to 755", 'wpflipwall') ?>. <br/>
		- <?php _e("The process that PHP runs under does not have enough permissions to create files.  Please contact your hosting provider for more details", 'wpflipwall') ?>. <br/>
	</div>
	...
	<table id="dup-log-panels">
		<tr>
			<td id="dup-log-panel-left">
				...
				<div class="opts"><a href="javascript:void(0)" id="dup-options"><?php _e("Options", 'wpflipwall') ?> <i class="fa fa-angle-double-right"></i></a> &nbsp;</div>
				<br style="clear:both" />
				<iframe id="dup-log-content" src="<?php echo $logurl ?>" ></iframe>							
			</td>
			<td id="dup-log-panel-right">
				<h2><?php _e("Options", 'wpflipwall') ?> </h2>
				<div class="dup-opts-items">
					<input type="button" class="button" id="dup-refresh" value="<?php _e("Refresh", 'wpflipwall') ?>" /> &nbsp; 
					<div style="display:inline-block; margin-top:5px">
						<input type='checkbox' id="dup-auto-refresh" style="margin-top:1px" /> 
						<label id="dup-auto-refresh-lbl" for="dup-auto-refresh">

							<?php _e("Auto Refresh", 'wpflipwall') ?>
							[<div id="dup-refresh-count"></div>]
						</label>
					</div>
				</div>

				<b><?php _e("Last 20 Logs", 'wpflipwall') ?> </b><br/>
				...
				<div class="dup-log-file-list">
					<?php 
						...
						foreach ($logs as $log) { 
							...
							$url  = '?page=flipwall-tools&logname=' . $name;
							echo ($active == $name) 
								? "<span title='{$name}'>{$time} - {$name}</span><br/>"
								: "<a href='javascript:void(0)'  title='{$name}' onclick='Flipwall.Tools.GetLog(\"{$url}\")'>{$time} - {$name}</a><br/>";
							if ($count > 20) break;
						} 
					?>
				</div>
